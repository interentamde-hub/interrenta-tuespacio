 ...
 *
 * Local change (InterRenta): the stall guard starts counting once the tab is
 * visible, so a page loaded in a background tab still animates. See `wake` below.
 */
...
export default function GlyphPortal({
  ...
  useLayoutEffect(() => {
    ...
    // A backgrounded tab withholds frames just like a hung font does, which would trip
    // the stall guard in `frame` and kill motion for the whole visit. Only start the
    // clock once frames can actually flow.
    const wake = () => {
      if (document.visibilityState !== "visible" || browserFrameSeen) return;
